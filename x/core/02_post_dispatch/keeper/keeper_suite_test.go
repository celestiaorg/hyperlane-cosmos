...
func createDummyPausableHook(s *i.KeeperTestSuite, creator string, mailboxId util.HexAddress) (util.HexAddress, error) {
	res, err := s.RunTx(&types.MsgCreatePausableHook{
		Owner:     creator,
		MailboxId: mailboxId,
	})
	if err != nil {
	...
	var response types.MsgCreatePausableHookResponse
	err = proto.Unmarshal(res.MsgResponses[0].Value, &response)
	if err != nil {
		return [32]byte{}, err
	}
